...
/**
  ...
  */
object GASimpleWithTransmogriAI extends OpAppWithRunner {
  ...
  val visitStartTime = FeatureBuilder.DateTime[Customer].extract(v => v.visitStartTime.map(_.toLong * 1000L).toDateTime).asPredictor
