...
*/
  ...
  def makeTrainRDD(df: DataFrame, dateTimeIndex: DateTimeIndex) = {
    ...
    val train_df_Int = assembler.transform(df)
           val train_df= train_df_Int.withColumn("user_id_str",getStrColumn($"user_id")).withColumn("o_area_Double",getDoubleColumn($"o_area")).withColumn("feature_vector",mlVector2MlibVector($"feature"))
            train_df.show(false)
...
//    val timeSeries_rdd: TimeSeriesRDD[Int] = new TimeSeriesRDD[Int](dateTimeIndex, user2feature_rdd)
  ...
  def GARCHMOdelTrain[K](trainTsrdd: TimeSeriesRDD[K])={
    val archRDD=trainTsrdd.map{case (key,denseVector)=>
      GARCH.fitModel(denseVector)
    }
  val foreast=archRDD.foreach{case model=>
  model.
  }
  }
  /**
