...
object TrainModels {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //验证结果模型
    trainModel.trainAndSaveModel("test")
    trainModel.varifyModel("test")
...
//      .withColumn("label_binary", udf_binary($"label_1"))
  ...
  def getResult(dataType:String="test") = {
    ...
    val submission: DataFrame = result.sort($"o_num".desc).limit(50000)
    ...
    println("数据不合法" + submission.filter($"pred_date".isNull).count())
