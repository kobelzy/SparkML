...
import com.cloudera.sparkts.models.{ARIMAModel, ARIMA}
...
class TimeSeriesModel extends Serializable{
   ...
   * @param sqlContext
   */
  private def saveInHive(dateDataRdd:RDD[Row],hiveColumnName:List[String],keyName:String,sqlContext:SQLContext,prop:Properties): Unit ={
...
//    dateDataDf.show(5)
    ...
    sqlContext.sql(sqlCommand).write.mode(SaveMode.Append).jdbc("jdbc:mysql://localhost:3306/mydata",outputTableName+"_"+keyName,prop)
  ...
  def actualForcastDateSaveInHive(trainTsrdd:TimeSeriesRDD[String],forecastValue:RDD[(String,Vector)],predictedN:Int,startTime:String,endTime:String,sc:SparkContext,hiveColumnName:List[String],keyName:String,sqlContext:SQLContext): Unit ={
    ...
    //在真实值后面追加预测值
    val forcast=forecastValue.map{case (key,forecastValue)=>(key,forecastValue.toArray.mkString(","))}
    ...
    for(i<-0 until actualAndForcastArray.length){
      ...
      if(dateDataRdd.collect()(0).toString()!="[1]"){
        saveInHive(dateDataRdd,hiveColumnName,keyName,sqlContext,prop)
      }
    }
  }
