...
def get_accumulate_action_feat(start_date, end_date):
    ...
    if os.path.exists(dump_path):
        ...
        actions = get_actions(start_date, end_date)
        df = pd.get_dummies(actions['type'], prefix='action')#哑变量
        actions = pd.concat([actions, df], axis=1) # type: pd.DataFrame#type的哑变量与源数据进行join
        #近期行为按时间衰减，使用结束时间-当前时间，越近，值越小。
